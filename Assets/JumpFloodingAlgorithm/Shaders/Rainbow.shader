    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target  /* 注意在HLSL中，fixed4类型变成了half4类型*/
            {
                ...
                float4 temp1 = saturate(fmod(abs((distance - _SinTime) * 12.0) + float4(0,4,2,0), 6) - 3);
                float4 temp2 = float4(1,1,1,0);
                float4 res = lerp(temp1,temp2,1-smoothstep(0.0,0.05,abs(distance)));
                return res;
            }
            ENDHLSL
        }
